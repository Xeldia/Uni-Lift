...
import { TerminalContainer } from "../../../shared/components/TerminalContainer";
import { TerminalInput } from "../../../shared/components/TerminalInput";
import { TerminalButton } from "../../../shared/components/TerminalButton";
import imageCompression from "browser-image-compression";
import { saveUserProfile, supabase, uploadAvatar, removeAvatar, uploadVerificationDoc } from "../../../shared/lib/supabase";

const VEHICLE_TYPES = ["MOTO", "CAR", "SIDECAR", "SHUTTLE"];
...
export default function Profile() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: profile, error: profileError } = await supabase
        .from("users")
        .select("id, email, full_name, student_id, phone_number, role, university, is_verified, account_status, driver_verification_status, rating, rides_completed, created_at, avatar_url, vehicle, vehicle_type, license_front_url, license_back_url, vehicle_reg_url")
      ...
      setDriverVerificationStatus(profile?.driver_verification_status ?? "");
      setEnrollmentDate(
        profile?.created_at ? new Date(profile.created_at).toLocaleDateString("en-US", { month: "long", year: "numeric" }) : "Unknown",
      );

      // Load avatar directly from the Supabase Storage public URL stored in users.avatar_url
      setPhotoPreview(profile?.avatar_url ?? "");
      setVerificationDocs({
        license_front_url: profile?.license_front_url ?? "",
        license_back_url: profile?.license_back_url ?? "",
        vehicle_reg_url: profile?.vehicle_reg_url ?? "",
      });
  ...
  const handleUploadPhoto = async (event) => {
    ...
    try {
      // ── Step 1: Compress client-side (max 800×800 px, WebP, < 300 KB) ──────
      ...
      // ── Step 2: Upload to Supabase Storage + update users.avatar_url ─────────
      let publicUrl;
      try {
        publicUrl = await uploadAvatar(authUser.id, compressed);
      } catch (err) {
        // Partial success: file is in storage but DB column write failed
        if (err?.publicUrl) {
          publicUrl = err.publicUrl;
          setError("Photo saved, but the profile link failed to update. Try saving your profile again.");
        } else {
          throw err;
        }
      }

      // ── Step 3: Instant UI update — no page refresh needed ───────────────────
      setPhotoPreview(publicUrl);
      setSavedConfirm("PHOTO UPDATED ✓");
    } catch (uploadError) {
  ...
  const handleRemovePhoto = async () => {
    if (!authUser || !photoPreview) return;
    ...
    try {
      await removeAvatar(authUser.id);
      setPhotoPreview("");
      setPhotoReference("");
  ...
  const handleUploadDoc = async (event, kind) => {
    if (!authUser) return;
    const selectedFile = event.target.files?.[0];
    event.target.value = "";
    if (!selectedFile) return;
    ...
    if (!["image/jpeg", "image/png", "image/webp", "application/pdf"].includes(selectedFile.type)) {
      setError("Supported verification formats: JPG, PNG, WebP, PDF.");
      return;
    }
    ...
    setSavedConfirm("");
    try {
      const publicUrl = await uploadVerificationDoc(authUser.id, selectedFile, kind);
      setVerificationDocs((prev) => ({ ...prev, [kind]: publicUrl }));
      setSavedConfirm("VERIFICATION DOCUMENT UPLOADED ✓");
    } catch (docError) {
      setError(docError.message || "Failed to upload verification document.");
    } finally {
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="w-full">
          ...
          <div className="px-[40px] py-[32px] flex flex-col gap-[28px]">
            ...
            {(accountStatus !== "ACTIVE" || driverVerificationStatus) && (
              <div className="flex flex-col gap-2">
                {accountStatus !== "ACTIVE" && (
                  <div className={`border px-3 py-2 flex items-center gap-2 ${
                    accountStatus === "SUSPENDED" ? "border-[#ef4444] bg-[#fef2f2]" :
                    accountStatus === "PENDING" ? "border-[#f59e0b] bg-[#fef3c7]" :
                    "border-black bg-[#f9f9f9]"
                  }`}>
                    <div className={`size-2 rounded-full ${
                      accountStatus === "SUSPENDED" ? "bg-[#ef4444]" :
                      accountStatus === "PENDING" ? "bg-[#f59e0b]" :
                      "bg-black"
                    }`} />
                    <p className="font-mono text-[9px] tracking-[0.5px]">
                      <span className="font-bold">ACCOUNT:</span> {accountStatus}
                      {accountStatus === "SUSPENDED" && " — Contact admin for reactivation."}
                ...
                {driverVerificationStatus && (
                  <div className={`border px-3 py-2 flex items-center gap-2 ${
                    driverVerificationStatus === "APPROVED" ? "border-[#10b981] bg-[#d1fae5]" :
                    driverVerificationStatus === "PENDING" ? "border-[#f59e0b] bg-[#fef3c7]" :
                    driverVerificationStatus === "REJECTED" ? "border-[#ef4444] bg-[#fef2f2]" :
                    "border-black bg-[#f9f9f9]"
                  }`}>
                    <div className={`size-2 rounded-full ${
                      driverVerificationStatus === "APPROVED" ? "bg-[#10b981]" :
                      driverVerificationStatus === "PENDING" ? "bg-[#f59e0b]" :
                      driverVerificationStatus === "REJECTED" ? "bg-[#ef4444]" :
                      "bg-black"
                    }`} />
                    <p className="font-mono text-[9px] tracking-[0.5px]">
                      <span className="font-bold">DRIVER VERIFICATION:</span> {driverVerificationStatus}
                      {driverVerificationStatus === "PENDING" && " — Documents under review."}
                      {driverVerificationStatus === "REJECTED" && " — Re-submit required documents."}
                      {driverVerificationStatus === "REVOKED" && " — Driver access removed by admin."}
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="flex flex-col items-center gap-[20px] pb-[32px] border-b border-black text-center mt-[12px]">
              <div className="relative shrink-0 group cursor-default">
                <div className="size-[84px] bg-black flex items-center justify-center relative shadow-[4px_4px_0px_0px_black] border-2 border-black overflow-hidden transition-transform duration-300 group-hover:-translate-y-[2px] group-hover:-translate-x-[2px]">
                  {photoPreview ? (
                    <img
                      src={photoPreview}
                      alt="Profile"
                      className={`w-full h-full object-cover transition-transform duration-500 ${uploadingPhoto ? "" : "group-hover:scale-110"}`}
                    />
                  ) : (
                    <p className="font-['Consolas:Bold',sans-serif] text-[28px] text-white transition-transform duration-300 group-hover:scale-110">{initials}</p>
                  )}
                ...
                <div className={`absolute -bottom-[6px] -right-[6px] size-[18px] border-[3px] border-white transition-transform duration-300 group-hover:-translate-y-[2px] group-hover:-translate-x-[2px] ${isVerified ? "bg-[#00c950]" : "bg-[#f59e0b]"}`} />
              </div>

              <div className="flex flex-col items-center max-w-[480px]">
                <p className="font-['Consolas:Bold',sans-serif] text-[24px] text-black tracking-[1.5px] mb-[10px]">{fullName.toUpperCase()}</p>
                <div className="flex flex-wrap items-center justify-center gap-[12px] mb-[14px]">
                  <div className="flex items-center gap-[6px]"><div className="bg-[#ffa500] size-[8px] rounded-full" /><p className="font-['Consolas:Medium',sans-serif] text-[10px] text-[#6a7282] tracking-[1px]">{role.toUpperCase()}</p></div>
                  <p className="font-['Consolas:Regular',sans-serif] text-[10px] text-[#6a7282] tracking-[0.5px]">ID: {studentId}</p>
                  <p className="font-['Consolas:Regular',sans-serif] text-[10px] text-[#6a7282] tracking-[0.5px]">{department}</p>
                  <div className="shrink-0 border border-black/20 px-[10px] h-[24px] flex items-center gap-[6px] ml-[4px] bg-[#f9f9f9]"><div className={`size-[6px] rounded-full ${isVerified ? "bg-[#00c950]" : "bg-[#f59e0b]"}`} /><p className="font-['Consolas:Medium',sans-serif] text-[8px] text-black tracking-[0.8px]">{isVerified ? "VERIFIED" : "PENDING"}</p></div>
                </div>
                <p className="font-['Consolas:Regular',sans-serif] text-[10px] text-[#6a7282] tracking-[0.5px] leading-[16px]">{bio || "No bio set yet."}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-[12px]">
              <div className="flex items-center gap-[12px] mb-[4px]">
                <p className="font-['Consolas:Bold',sans-serif] text-[11px] text-black tracking-[1.5px] whitespace-nowrap">
                  VEHICLE MANAGEMENT
                </p>
                <div className="flex-1 h-px bg-black opacity-10" />
              </div>
              <p className="font-['Consolas:Regular',sans-serif] text-[9px] text-[#6a7282] tracking-[0.45px]">
                Required before driver mode can go online or take orders.
              </p>
              <div className="grid grid-cols-2 gap-[14px]">
                <TerminalInput
                  label="VEHICLE MODEL / PLATE"
                  icon={ICON_ID}
                  value={vehicle}
                  onChange={(e) => setVehicle(e.target.value)}
                />
                <div className="flex flex-col gap-[6px]">
                  <p className="font-['Consolas:Medium',sans-serif] text-[9px] text-black tracking-[1.5px]">VEHICLE TYPE</p>
                  <select
                    value={vehicleType}
                    onChange={(e) => setVehicleType(e.target.value)}
                    className="border border-black h-[42px] px-[13px] font-['Consolas:Regular',sans-serif] text-[11px] text-black tracking-[0.55px] bg-white outline-none"
                  >
            ...
            <div className="flex flex-col gap-[12px]">
              ...
              <p className="font-['Consolas:Regular',sans-serif] text-[9px] text-[#6a7282] tracking-[0.45px]">
                Upload license and vehicle registration to speed up approval for driver mode.
              </p>
              <div className="grid grid-cols-3 gap-[12px]">
                {[
                  ...
                  <label key={item.key} className="border border-black p-[12px] flex flex-col gap-[8px] cursor-pointer hover:bg-[#f9f9f9] transition-colors">
                    <input
                      type="file"
                      accept=".jpg,.jpeg,.png,.webp,.pdf,image/jpeg,image/png,image/webp,application/pdf"
                      className="hidden"
                      disabled={uploadingDocs}
                      onChange={(e) => handleUploadDoc(e, item.key)}
                    />
                    <p className="font-['Consolas:Bold',sans-serif] text-[8px] text-black tracking-[0.8px]">{item.label}</p>
                    <p className="font-['Consolas:Regular',sans-serif] text-[8px] text-[#6a7282]">
                      {verificationDocs[item.key] ? "Uploaded" : "Click to upload"}
                    </p>
                  </label>
                ))}
              </div>
            ...
            <div className="flex flex-col gap-[12px]">
              ...
              <div className="grid grid-cols-4 gap-[12px]">
                {[
                  ...
                  <div key={i} className="border border-black p-[16px] bg-white transition-all duration-300 hover:-translate-y-[2px] hover:-translate-x-[2px] hover:shadow-[4px_4px_0px_0px_black] group cursor-default">
                    <p className="font-['Consolas:Bold',sans-serif] text-[24px] text-black mb-[6px] leading-none group-hover:scale-[1.05] transition-transform origin-left">{stat.value}</p>
                    <p className="font-['Consolas:Regular',sans-serif] text-[8px] text-[#6a7282] tracking-[0.8px] mb-[4px]">{stat.label}</p>
                    <p className="font-['Consolas:Regular',sans-serif] text-[7px] text-[#99a1af] tracking-[0.4px] transition-colors group-hover:text-black/60">{stat.sub}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-[12px]">
              ...
              <div className="border border-black">
                ...
                {(recentRides.length ? recentRides : [{ from: "N/A", to: "No rides yet", date: "-", fare: "-", status: "NONE" }]).map((ride, i, arr) => (
                  <div key={i} className={`flex items-center px-[16px] py-[12px] gap-[12px] hover:bg-[#f9f9f9] transition-colors cursor-pointer group ${i < arr.length - 1 ? "border-b border-black/10" : ""}`}>
                    <div className="flex-[2_0_0] min-w-0"><p className="font-['Consolas:Medium',sans-serif] text-[9px] text-black tracking-[0.45px] truncate group-hover:font-['Consolas:Bold',sans-serif] transition-all">{ride.from}<span className="text-[#99a1af] mx-[6px] group-hover:text-black transition-colors">→</span>{ride.to}</p></div>
